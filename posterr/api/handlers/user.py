...
class UserHandlers(BaseHandler, View):
    ...
    def _is_valid_post_body(self, name: str) -> Tuple[bool, str]:
        if not name:
            return (
                False,
                "At least you need to give a name to create a user my friend",
            )
        return (True, "Fine :D")
    ...
    def _get_body(self, body: dict) -> List[str]:
        body = [body.get("action", ""), body.get("following", ""), body.get("name", "")]
        return body

    def _is_valid_put_body(
        self, action: str, following_id: str, user_name: str
    ) -> Tuple[bool, str]:
        if not action:
            return (
                False,
                "It is necessary to give one of the following actions: 'UPDATE', 'FOLLOW', 'UNFOLLOW'",
            )
